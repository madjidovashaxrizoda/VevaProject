...
export default function UserNameBtn() {
    const LogInModal = function () {
		document.getElementById('footer').classList.add('blur-xl')
  
    document.getElementById('backdrop').classList.add('blur-xl')
		document.getElementById('modal').classList.remove('hidden')
	}
  const close = function () {
    
    document.getElementById('footer').classList.remove('blur-xl')
    document.getElementById('modal').classList.add('hidden')
    document.getElementById('backdrop').classList.remove('blur-xl')
}
  return (
    <div>
      <div>
      <button id="LogInBtn" onClick={LogInModal}    class="  text-black bg-white font-normal  rounded-none text-sm px-6	 py-1.5 text-center flex gap-5  items-center max-[650px]:rounded-full max-[650px]:px-4 max-[650px]:py-3 max-[420px]:py-2 max-[420px]:px-3 max-[360px]:px-2 max-[360px]:py-1" type="button">
    ...
    <div id="modal" class=" fixed hidden left-1/3 top-36 bg-white rounded-2xl 
    ...
    <form class=" mx-auto flex-col items-center pt-14 justify-center flex gap-6 py-6 px-8 box-border max-[650px]:gap-4">
      ...
      <button class="text-lg font-light underline text-center pb-10 max-[650px]:pb-0 max-[650px]:text-[15px]  ">Зарегистрироваться</button>
    </form>
